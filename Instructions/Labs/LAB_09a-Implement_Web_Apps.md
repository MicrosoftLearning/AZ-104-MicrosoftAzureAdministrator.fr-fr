...
1. Dans le Portail Azure, recherchez et sélectionnez **App Services**, puis, dans le volet **App Services**, cliquez sur **+ Créer**.

1. Sous l’onglet **Informations de base** du volet **Créer une application web**, spécifiez les paramètres suivants (laissez les valeurs par défaut des autres paramètres) :
    ...
    | App service plan (Plan App Service) | acceptez la configuration par défaut |

1. Cliquez sur **Vérifier + créer**. Sous l’onglet **Vérifier + créer** du volet **Créer une application web**, vérifiez que la validation a réussi et cliquez sur **Créer**.
...
1. Dans le volet de l’application web que vous venez de déployer, cliquez sur le lien **URL** pour afficher la page web par défaut dans un nouvel onglet de navigateur.
...
1. De retour dans le volet **Emplacements de déploiement** de l’application web, cliquez sur l’entrée représentant l’emplacement de préproduction que vous venez de créer.
    ...
    >**Remarque** : Cette opération va ouvrir le volet affichant les propriétés de l’emplacement de préproduction.

1. Passez en revue le volet de l’emplacement de préproduction et notez que son URL diffère de celle affectée à l’emplacement de production.
...
1. Dans le volet de l’emplacement de déploiement de transit, dans la section **Déploiement**, cliquez sur **Centre de déploiement**, puis sélectionnez l’onglet **Paramètres**.

    >**Remarque :** Vérifiez que vous êtes sur le volet de l’emplacement de préproduction (plutôt que sur l’emplacement de production).
    
1. Sous l’onglet **Paramètres**, dans la liste déroulante **Source**, sélectionnez **Git local**, puis cliquez sur le bouton **Enregistrer**

1. Dans le volet **Centre de déploiement**, copiez l’entrée **URL Git Clone** dans le Bloc-notes.

    >**Remarque :** Vous aurez besoin de la valeur d’URL Git Clone dans la tâche suivante de ce labo.

1. Dans le volet **Centre de déploiement**, sélectionnez l’onglet **Informations d’identification Git/FTPS local**. Dans la section **Étendue utilisateur**, spécifiez les paramètres suivants, puis cliquez sur **Enregistrer**.
...
Dans cette tâche, vous allez déployer du code sur l’emplacement de déploiement de transit.
...
1. Dans le volet Cloud Shell, exécutez la commande suivante pour cloner le référentiel distant contenant le code de l’application web.

   ```powershell
   git clone https://github.com/Azure-Samples/php-docs-hello-world
   ```

1. Dans le volet Cloud Shell, exécutez la commande suivante pour définir l’emplacement actuel sur le clone que vous venez de créer du référentiel local contenant l’exemple de code d’application web.
...
1. Dans le volet de l’emplacement de préproduction, cliquez sur **Vue d’ensemble**, puis cliquez sur le lien **URL** pour afficher la page web par défaut dans un nouvel onglet de navigateur.
...
1. Revenez au volet affichant l’emplacement de production de l’application web.

1. Dans la section **Déploiement**, cliquez sur **Emplacements de déploiement**, puis sur l’icône **Permuter** de la barre d’outils.

1. Dans le volet **Permuter**, passez en revue les paramètres par défaut, puis cliquez sur **Permuter**.

1. Cliquez sur **Vue d’ensemble** dans le volet de l’emplacement de production de l’application web, puis cliquez sur le lien **URL** pour afficher la page d’accueil du site web dans un nouvel onglet de navigateur.
...
1. Dans le volet affichant l’emplacement de production de l’application web, dans la section **Paramètres**, cliquez sur **Scale-out (plan App Service)** .
...
1. Laissez l’option par défaut **Mise à l’échelle selon une métrique** sélectionnée, puis cliquez sur **+ Ajouter une règle**

1. Dans le volet **Règle de mise à l’échelle**, spécifiez les paramètres suivants (conservez les valeurs par défaut pour les autres) :
...
1. Cliquez sur **Ajouter** et, dans le volet de mise à l’échelle du plan App Service, spécifiez les paramètres suivants (laissez les autres avec leurs valeurs par défaut) :
...
1. Dans le volet Cloud Shell, exécutez la commande suivante pour identifier l’URL de l’application web Azure.
...
1. Dans le volet Cloud Shell, exécutez la commande suivante pour démarrer et exécuter une boucle infinie qui envoie les requêtes HTTP à l’application web :

   ```powershell
   while ($true) { Invoke-WebRequest -Uri $webapp.DefaultHostName }
   ```

1. Réduisez le volet Cloud Shell (sans le fermer) et, dans le volet de l’application web, dans la section **Surveillance**, cliquez sur **Explorateur de processus**.
...
1. Une fois que vous avez remarqué que le nombre d’instances est passé à 2, rouvrez le volet Cloud Shell et terminez le script en appuyant sur **Ctrl+C**.
